      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8 text-sm font-semibold">
          {menuItems.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-1 cursor-pointer px-3 py-1.5 rounded-full hover:bg-white hover:text-[#FF8A33] transition-all duration-200">
                {item.name}
              ...
              {item.submenu.length > 0 && (
                <ul
                  className={`absolute top-full left-0 mt-2 bg-white text-[#FF8A33] border border-orange-200 rounded-xl shadow-xl overflow-hidden min-w-[160px] transition-all duration-200 ${
                    activeIndex === index
        ...
        {/* CTA Button */}
        <button className="bg-white text-[#FF8A33] px-5 py-2.5 rounded-full text-sm font-semibold shadow hover:shadow-lg hover:bg-orange-100 transition-all duration-200">
          Get the App
        </button>
      </div>
    </nav>
